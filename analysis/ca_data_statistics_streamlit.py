...
df_selection_inliers_0 = pd.DataFrame(False, index=[df_selection_all_col_1.set_index('isin').index], columns=delta_years_str + ['last_available_year'])
...
if threshold_1 is not None:
    if comparison_1 == 'superior':
        ...
        df_nmr_inliers_0 = (df_selection_1[delta_years_str] > threshold_1).groupby(df_selection_1["scope"]).sum()
        df_selection_inliers_0 = (df_selection_all_col_1.set_index('isin')[delta_years_str + ['last_available_year']].copy() > threshold_1) 
...
df_selection_outliers_0_3 = pd.DataFrame(
    True, index=df_selection_all_col_1.set_index('isin').index.intersection(
        df_selection_all_col_3.set_index('isin').index), columns=delta_years_str + ['last_available_year'])
...
if threshold_2 is not None:
    if comparison_2 == 'superior':
        df_selection_inliers_3 = (df_selection_all_col_3.set_index('isin')[delta_years_str + ['last_available_year']]> threshold_2)
        df_selection_outliers_3 = (df_selection_all_col_3.set_index('isin')[delta_years_str + ['last_available_year']] <= threshold_2)
    ...
    if (threshold_1 is not None) and (comparison_1 is not None) and (comparison_2 is not None): 
        ...
        df_selection_outliers_0_inliers_3 = (df_selection_outliers_0 & df_selection_inliers_3)
        df_selection_inliers_0_3 = (df_selection_inliers_0 | df_selection_inliers_3)
        df_nmr_inliers_2 = df_selection_outliers_0_inliers_3.sum().rename(selected_scope_0).loc[delta_years_str]
...
# ----------------------------
if len(selected_hisector_0)==1 & (selected_hisector_0[0] == 'Automobiles') & ((selected_scope_0=="s3_d") or (selected_scope_0=="s3")):
    ...
    df_selection_outliers_0_inliers_5 = pd.DataFrame(
        True, index=df_selection_all_col_1.set_index('isin').index.intersection(
            df_selection_all_col_5.set_index('isin').index), columns=delta_years_str + ['last_available_year'])
    ...
    with col1:
        st.write("Among the selection (for absolute emissions), number of them with automobiles intensities by km growth rate")

    with col2:
        comparison_3 = st.radio(label="comparison_3", options=["superior", "inferior"], 
                                horizontal=True, key='comparison_3', label_visibility="collapsed")
    ...
    with col4:
        threshold_3 = st.selectbox(
            label="threshold_3", options=[None, -0.3, -0.2, -0.1, 0.0, 0.1, 0.2, 0.3],
            format_func=lambda x: "Select a value" if x is None else f"{x:.1%}", 
            key='threshold_3', label_visibility="collapsed")

    if threshold_3 is not None:
        if comparison_3 == 'superior':
            df_selection_inliers_5 = (df_selection_all_col_5.set_index('isin')[delta_years_str + ['last_available_year']] > threshold_3)
            df_selection_outliers_5 = (df_selection_all_col_5.set_index('isin')[delta_years_str + ['last_available_year']] <= threshold_3)
        elif comparison_3 == "inferior":
            df_selection_inliers_5 = (df_selection_all_col_5.set_index('isin')[delta_years_str + ['last_available_year']] <= threshold_3)
            df_selection_outliers_5 = (df_selection_all_col_5.set_index('isin')[delta_years_str + ['last_available_year']] > threshold_3)
        if (threshold_1 is not None) and (comparison_1 is not None) and (comparison_3 is not None): 
            df_selection_outliers_0_5 = (df_selection_outliers_0 & df_selection_outliers_5)
            df_selection_outliers_0_inliers_5 = (df_selection_outliers_0 & df_selection_inliers_5)
            df_nmr_outliers_3 = df_selection_outliers_0_inliers_5.sum().rename(selected_scope_0).loc[delta_years_str]
...
df_selection_3 = df_selection_all_col_3.set_index('isin')[delta_years_str + ['last_available_year']].copy().where(df_selection_outliers_0_inliers_3)
df_selection_3 = pd.concat([df_selection_all_col_3.set_index('isin')['scope'], df_selection_3], axis=1)
...
if len(selected_hisector_0)==1 & (selected_hisector_0[0] == 'Automobiles') & ((selected_scope_0=="s3_d") or (selected_scope_0=="s3")):
    ...
    st.write("**Automobiles intensities (emissions / km) growth rate - statistics** among the selectionned equities")

    df_selection_5 = df_selection_all_col_5.set_index('isin')[delta_years_str + ['last_available_year']].copy().where(df_selection_outliers_0_inliers_5)
    df_selection_5 = pd.concat([df_selection_all_col_5.set_index('isin')['scope'], df_selection_5], axis=1)

    selected_year_6 = st.multiselect(
        label='selected_year_6', label_visibility="collapsed", options=['last_available_year'] + delta_years_str, 
        placeholder="Choose one or multiple years range(s)", key='year_6')

    if not selected_year_6:
        selected_year_6 = delta_years_str

    df_selection_5_mean = pd.concat([df_selection_5['scope'], df_selection_5[selected_year_6].mean(axis=1).rename("mean_value")], axis=1)
...
df_fmt_6 = pd.DataFrame([{'with targets': df_selection_1_targets['has_selected_target'].sum(), 'total': df_selection_1_targets['isin'].count()}]).T
...
if abs_outliers:
    ...
    df_selection_inliers_1_targets = pd.concat([df_selection_inliers_1, df_targets.loc[df_selection_inliers_1.index]], axis=1)
    df_selection_inliers_1_targets['has_selected_target'] = df_selection_inliers_1_targets[selected_targets].fillna(False).any(axis=1)
    df_selection_inliers_1_targets = df_selection_inliers_1_targets.drop(list(dict_mapping_targets_ctgry.values()), axis=1)

    df_fmt_6 = pd.concat([df_selection_inliers_1_targets[df_selection_inliers_1_targets['has_selected_target']][delta_years_str].sum(),
            df_selection_inliers_1_targets[delta_years_str].sum()], axis=1, keys=['with targets', 'total']).T
