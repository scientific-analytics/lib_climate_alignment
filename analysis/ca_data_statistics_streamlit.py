...
# ----------------------------
# Run Streamlit:
# streamlit run analysis/ca_data_statistics_streamlit.py
# ----------------------------
...
# ----------------------------
# TABLE 5 - Absolute emissions - growth rate statistics
# ----------------------------

st.write("**Absolute emissions growth rate - statistics**")

df_selection_all_col_1 = df_eq_abs_em_rate[
...
df_selection_all_col_1["last_available_year"] = (
    df_selection_all_col_1[delta_years_str[::-1]].bfill(axis=1).iloc[:, 0]
)

df_selection_1 = df_selection_all_col_1[
    ["scope"] + delta_years_str + ["last_available_year"]
].copy()

selected_year_1 = st.multiselect(
    label="selected_year_1",
    label_visibility="collapsed",
    options=["last_available_year"] + delta_years_str,
    placeholder="Choose one or multiple years range(s)",
    key="year_1",
)
...
df_selection_1_mean = pd.concat(
    [
        df_selection_1["scope"],
        df_selection_1[selected_year_1].mean(axis=1).rename("mean_value"),
    ],
    axis=1,
)
...
# ----------------------------
# TABLE 6 - Absolute emissions growth rate - nbr that pass
# ----------------------------
st.write("**Absolute emissions growth rate - number in the selection**")

df_nmr_outliers_0 = pd.DataFrame(index=[selected_scope_0], columns=delta_years_str)
df_nmr_inliers_0 = pd.DataFrame(index=[selected_scope_0], columns=delta_years_str)
df_selection_outliers_0 = pd.DataFrame(
    False,
    index=[df_selection_all_col_1.set_index("isin").index],
    columns=delta_years_str + ["last_available_year"],
)
df_selection_inliers_0 = pd.DataFrame(
    False,
    index=[df_selection_all_col_1.set_index("isin").index],
    columns=delta_years_str + ["last_available_year"],
)
...
if threshold_1 is not None:
    if comparison_1 == "superior":
        df_nmr_outliers_0 = (
            (df_selection_1[delta_years_str] <= threshold_1)
            .groupby(df_selection_1["scope"])
            .sum()
        )
        df_nmr_inliers_0 = (
            (df_selection_1[delta_years_str] > threshold_1)
        ...
        df_selection_inliers_0 = (
            df_selection_all_col_1.set_index("isin")[
                delta_years_str + ["last_available_year"]
            ].copy()
            > threshold_1
        )
    ...
    df_nmr_inliers_0.index = [f"nbr selected for {selected_scope_0}"]
    df_nmr_inliers_0 = pd.concat([total_counts, df_nmr_inliers_0])
    df_nmr_outliers_0.index = [f"nbr selected for {selected_scope_0}"]
    df_nmr_outliers_0 = pd.concat([total_counts, df_nmr_outliers_0])

st.dataframe(df_nmr_inliers_0, height=row_height * (len(df_nmr_inliers_0) + 1))


# ----------------------------
# TABLE 7 - Intensity that passs Among abs emitter outlier
# ----------------------------

df_selection_all_col_3 = df_eq_intensities_rate[
...
df_selection_all_col_3["last_available_year"] = (
    df_selection_all_col_3[delta_years_str[::-1]].bfill(axis=1).iloc[:, 0]
)

df_selection_outliers_0_3 = pd.DataFrame(
    True,
    index=df_selection_all_col_1.set_index("isin").index.intersection(
        df_selection_all_col_3.set_index("isin").index
    ),
    columns=delta_years_str + ["last_available_year"],
)
df_selection_outliers_0_inliers_3 = pd.DataFrame(
    True,
    index=df_selection_all_col_1.set_index("isin").index.intersection(
        df_selection_all_col_3.set_index("isin").index
    ),
...
df_nmr_inliers_2 = df_nmr_outliers_0.copy()

col1, col2, col3, col4 = st.columns([3, 2, 1, 3])

with col1:
    st.write("Among the **not selected**, number of them with intensities growth rate")

with col2:
    comparison_2 = st.radio(
        label="comparison_2",
        options=["superior", "inferior"],
        horizontal=True,
        key="comparison_2",
...
if threshold_2 is not None:
    if comparison_2 == "superior":
        df_selection_inliers_3 = (
            df_selection_all_col_3.set_index("isin")[
                delta_years_str + ["last_available_year"]
            ]
            > threshold_2
        )
        df_selection_outliers_3 = (
            df_selection_all_col_3.set_index("isin")[
                delta_years_str + ["last_available_year"]
            ]
            <= threshold_2
    ...
    if (
        ...
        and (comparison_1 is not None)
        and (comparison_2 is not None)
    ):
        df_selection_outliers_0_3 = df_selection_outliers_0 & df_selection_outliers_3
        df_selection_outliers_0_inliers_3 = (
            df_selection_outliers_0 & df_selection_inliers_3
        )
        df_selection_inliers_0_3 = df_selection_inliers_0 | df_selection_inliers_3
        df_nmr_inliers_2 = (
            df_selection_outliers_0_inliers_3.sum()
            .rename(selected_scope_0)
            .loc[delta_years_str]
        )
        df_nmr_inliers_2 = df_nmr_inliers_2.to_frame().T
elif (threshold_1 is not None) and (comparison_1 is not None):
    df_selection_outliers_0_inliers_3 = df_selection_outliers_0
    df_selection_inliers_0_3 = df_selection_inliers_0

st.dataframe(df_nmr_inliers_2)

# ----------------------------
# TABLE 8 - BONUS - Automobile Intensity outlier Among abs emitter outlier
# ----------------------------
if len(selected_hisector_0) == 1 & (selected_hisector_0[0] == "Automobiles") & (
    (selected_scope_0 == "s3_d") or (selected_scope_0 == "s3")
):

    df_selection_all_col_5 = df_eq_auto_intensities_rate[
    ...
    df_selection_all_col_5["last_available_year"] = (
        df_selection_all_col_5[delta_years_str[::-1]].bfill(axis=1).iloc[:, 0]
    )

    df_selection_outliers_0_inliers_5 = pd.DataFrame(
        True,
        index=df_selection_all_col_1.set_index("isin").index.intersection(
            df_selection_all_col_5.set_index("isin").index
        ),
        columns=delta_years_str + ["last_available_year"],
    )

    df_nmr_outliers_3 = df_nmr_outliers_0.copy()

    col1, col2, col3, col4 = st.columns([3, 2, 1, 3])

    with col1:
        st.write(
            "Among the selection (for absolute emissions), number of them with automobiles intensities by km growth rate"
        )

    with col2:
        comparison_3 = st.radio(
            label="comparison_3",
            options=["superior", "inferior"],
            horizontal=True,
            key="comparison_3",
    ...
    if threshold_3 is not None:
        if comparison_3 == "superior":
            df_selection_inliers_5 = (
                df_selection_all_col_5.set_index("isin")[
                    delta_years_str + ["last_available_year"]
                ]
                > threshold_3
            )
            df_selection_outliers_5 = (
                df_selection_all_col_5.set_index("isin")[
                    delta_years_str + ["last_available_year"]
                ]
                <= threshold_3
            )
        elif comparison_3 == "inferior":
            df_selection_inliers_5 = (
                df_selection_all_col_5.set_index("isin")[
                    delta_years_str + ["last_available_year"]
                ]
        ...
        if (
            (threshold_1 is not None)
            and (comparison_1 is not None)
            and (comparison_3 is not None)
        ):
            df_selection_outliers_0_5 = (
                df_selection_outliers_0 & df_selection_outliers_5
            )
            df_selection_outliers_0_inliers_5 = (
                df_selection_outliers_0 & df_selection_inliers_5
            )
            df_nmr_outliers_3 = (
                df_selection_outliers_0_inliers_5.sum()
                .rename(selected_scope_0)
                .loc[delta_years_str]
            )
            df_nmr_outliers_3 = df_nmr_outliers_3.to_frame().T
    elif (threshold_1 is not None) and (comparison_1 is not None):
        df_selection_outliers_0_inliers_5 = df_selection_outliers_0

    st.dataframe(df_nmr_outliers_3)

# ----------------------------
# TABLE 9 - Intensities - growth rate statistics
# ----------------------------

st.write("**Intensities growth rate - statistics** among the equities displayed above")

# df_selection_3 = df_selection_all_col_3[['scope'] + delta_years_str + ['last_available_year']].copy()
df_selection_3 = (
    df_selection_all_col_3.set_index("isin")[delta_years_str + ["last_available_year"]]
    .copy()
    .where(df_selection_outliers_0_inliers_3)
)
df_selection_3 = pd.concat(
    [df_selection_all_col_3.set_index("isin")["scope"], df_selection_3], axis=1
)

selected_year_3 = st.multiselect(
    label="selected_year_3",
    label_visibility="collapsed",
    options=["last_available_year"] + delta_years_str,
    placeholder="Choose one or multiple years range(s)",
    key="year_3",
...
df_selection_3_mean = pd.concat(
    [
        df_selection_3["scope"],
        df_selection_3[selected_year_3].mean(axis=1).rename("mean_value"),
    ],
    axis=1,
)
df_stats_3 = df_selection_3_mean.groupby("scope")["mean_value"].describe().fillna(0)
...
# ----------------------------
# TABLE 10 - BONUS - Intensities by km - growth rate statistics -
# ----------------------------
if len(selected_hisector_0) == 1 & (selected_hisector_0[0] == "Automobiles") & (
    (selected_scope_0 == "s3_d") or (selected_scope_0 == "s3")
):
    ...
    st.write(
        "**Automobiles intensities (emissions / km) growth rate - statistics** among the selectionned equities"
    )
    ...
    df_selection_5 = (
        ...
        .copy()
        .where(df_selection_outliers_0_inliers_5)
    )
    df_selection_5 = pd.concat(
        [df_selection_all_col_5.set_index("isin")["scope"], df_selection_5], axis=1
    )

    selected_year_6 = st.multiselect(
        label="selected_year_6",
        label_visibility="collapsed",
        options=["last_available_year"] + delta_years_str,
        placeholder="Choose one or multiple years range(s)",
        key="year_6",
    ...
    if not selected_year_6:
        selected_year_6 = delta_years_str

    df_selection_5_mean = pd.concat(
        [
            df_selection_5["scope"],
            df_selection_5[selected_year_6].mean(axis=1).rename("mean_value"),
        ],
        axis=1,
    )
    df_stats_5 = df_selection_5_mean.groupby("scope")["mean_value"].describe()
    ...
    df_fmt_5[cols_pct] = df_stats_5[cols_pct].map(lambda x: f"{x:.1%}")
...
st.write("**Number with targets, among the selection**")
...
df_targets = df.query('scope=="s1"')[
    ["isin"] + list(dict_mapping_targets_ctgry.values())
].set_index("isin")
...
def sync_toggle():
    if st.session_state.int_outliers:
        st.session_state.abs_outliers = True


with col2:
    abs_outliers = st.toggle("Selected with absolute emissions rates")

with col3:
    int_outliers = st.toggle("Selected with intensities rates")
    on_change = sync_toggle


selected_targets = [dict_mapping_targets_ctgry[lbl] for lbl in selected_labels]
df_selection_1_targets = pd.merge(
    df_selection_all_col_1, df_targets.reset_index(), on="isin"
)
df_selection_1_targets["has_selected_target"] = (
    df_selection_1_targets[selected_targets].fillna(False).any(axis=1)
)
df_fmt_6 = pd.DataFrame(
    [
        {
            "with targets": df_selection_1_targets["has_selected_target"].sum(),
            "total": df_selection_1_targets["isin"].count(),
        }
    ]
).T
...
if abs_outliers:
    if int_outliers:
        df_selection_inliers_1 = df_selection_inliers_0_3.copy()
    else:
        df_selection_inliers_1 = df_selection_inliers_0.copy()

    df_selection_inliers_1_targets = pd.concat(
        [df_selection_inliers_1, df_targets.loc[df_selection_inliers_1.index]], axis=1
    )
    df_selection_inliers_1_targets["has_selected_target"] = (
        df_selection_inliers_1_targets[selected_targets].fillna(False).any(axis=1)
    )
    df_selection_inliers_1_targets = df_selection_inliers_1_targets.drop(
        list(dict_mapping_targets_ctgry.values()), axis=1
    )

    df_fmt_6 = pd.concat(
        [
            df_selection_inliers_1_targets[
                df_selection_inliers_1_targets["has_selected_target"]
            ][delta_years_str].sum(),
            df_selection_inliers_1_targets[delta_years_str].sum(),
        ],
        axis=1,
        keys=["with targets", "total"],
    ).T

st.dataframe(df_fmt_6, height=row_height * (len(df_fmt_6) + 1))
...
selected_year_4 = st.multiselect(
    label="selected_year_4",
    options=["last_available_year"] + delta_years_str,
    placeholder="Choose a year range",
    key="year_4",
    label_visibility="collapsed",
)
...
df_selection_all_col_1["mean_selected_year"] = df_selection_all_col_1[
    selected_year_4
].mean(axis=1)
df_selection_all_col_3["mean_selected_year"] = df_selection_all_col_3[
    selected_year_4
].mean(axis=1)

df_selection_all_col_4 = pd.merge(
    df_selection_all_col_1[["isin", "mean_selected_year"]].rename(
        columns={"mean_selected_year": "abs"}
    ),
    df_selection_all_col_3[["isin", "mean_selected_year"]].rename(
        columns={"mean_selected_year": "int"}
    ),
    on="isin",
)
...
st.write(
    "Note: This graph shows the average growth rates for the selected years. For visual clarity, the axes are truncated at 200%, although certain observations lie above this threshold."
)
...
df_selection_ratio = df_eq_abs_em[
    (df_eq_abs_em["region_0"].isin(selected_region_0))
    & (df_eq_abs_em["high_impact_sector"].isin(selected_hisector_0))
]
...
df_selection_ratio["mean_selected_year"] = df_selection_ratio[selected_year_5].mean(
    axis=1
)
...
for scope_nu, scope_de in list_ratios:
    df_ratio = (
        ...
        .groupby("isin")
        .apply(
            lambda g: g.loc[g["scope"] == scope_nu, "mean_selected_year"].iloc[0]
